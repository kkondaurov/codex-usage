...
test('keyboard focus, filter clearing, tabs, and Activity disclosures remain honest and visible', async ({ page, app }) => {
  ...
  const projectMenu = page.getByRole('menu', { name: 'Projects' })
  await expect(projectMenu).toBeVisible()
  const projectOptions = projectMenu.getByRole('menuitemradio')
  await expect(projectOptions.first()).toBeFocused()
  expect(await projectOptions.evaluateAll(options => options.map(option => (option as HTMLElement).tabIndex)))
    .toEqual([0, ...Array(Math.max(0, await projectOptions.count() - 1)).fill(-1)])
  await page.keyboard.press('Tab')
  await expect(projectMenu).toBeHidden()
  await expect(page.locator('[role="menuitemradio"]:focus')).toHaveCount(0)
