...
mod tests {
    ...
    #[test]
    fn cursor_wire_shape_and_scope_error_are_exact() {
        let row = super::IndexedActivityRow {
            event_id: "event".to_owned(),
            timestamp: "2026-01-02T03:04:05.000000000Z".to_owned(),
            source_line: 7,
        };
        let cursor = super::encode_cursor(&row, "thread", "turn").unwrap();
        assert_eq!(
            cursor,
            r#"{"version":1,"thread_id":"thread","turn_id":"turn","timestamp":"2026-01-02T03:04:05.000000000Z","source_line":7,"event_id":"event"}"#
        );
        assert_eq!(
            super::decode_cursor(&cursor).unwrap(),
            super::ActivityCursor {
                version: 1,
        ...
        assert_eq!(
            validate_cursor_for(&cursor, "thread", "another-turn")
                .unwrap_err()
                .to_string(),
            "Activity cursor belongs to a different turn"
        );
    }
