...
mod tests {
    ...
    fn manual_config_survives_projection_deletion_as_fixed_point_json() {
        ...
        db.manual_pricing()
            .save_price(&db, price("manual-model"))
            .unwrap();
        db.manual_pricing()
            .save_alias(
                &db,
        ...
        let rebuilt = Db::open_with_pricing_config(&db_path, &config_path).unwrap();
    ...
    fn manual_pricing_config_is_owner_only() {
        ...
        let db = Db::open_with_pricing_config(&db_path, &config_path).unwrap();
        db.manual_pricing()
            .save_price(&db, price("private-price"))
            .unwrap();
    ...
    fn hydration_repairs_permissive_existing_config_permissions() {
        ...
        let _db = Db::open_with_pricing_config(&db_path, &config_path).unwrap();
    ...
    fn advisory_lock_serializes_independent_processes() {
        ...
        let mut child = Command::new(std::env::current_exe().unwrap())
            .arg("--exact")
            .arg("manual_pricing::tests::pricing_lock_child")
            .arg("--nocapture")
    ...
    fn pricing_lock_child() {
        ...
        let db = Db::open_with_pricing_config(db_path, config_path).unwrap();
        db.manual_pricing()
            .save_price(&db, price("child-process-price"))
            .unwrap();
    }
    ...
    fn alias_validation_rejects_chains_in_both_insertion_orders() {
        ...
        let db = Db::open(temp.path().join("usage.db")).unwrap();
        db.manual_pricing()
            .save_price(&db, price("canonical"))
            .unwrap();
        db.manual_pricing()
            .save_price(&db, price("middle"))
            .unwrap();
        db.manual_pricing()
            .save_alias(
                &db,
            ...
            .unwrap();
        let reverse = db.manual_pricing().save_alias(
            &db,
            ManualAlias {
        ...
        db.manual_pricing().delete_alias(&db, "first").unwrap();
        db.manual_pricing()
            .save_alias(
                &db,
                ManualAlias {
            ...
            .unwrap();
        let forward = db.manual_pricing().save_alias(
            &db,
            ManualAlias {
                observed_model_id: "first".into(),
                canonical_model_id: "middle".into(),
    ...
    fn deleting_manual_override_restores_bundled_fallback_immediately() {
        ...
        db.manual_pricing().save_price(&db, override_price).unwrap();
        db.manual_pricing()
    ...
    fn fixed_width_effective_timestamp_prices_usage_at_the_exact_instant() {
        ...
        db.manual_pricing().save_price(&db, exact).unwrap();
        let connection = db.connect().unwrap();
        connection
    ...
    fn deleting_last_price_cannot_leave_a_dangling_alias() {
        ...
        let db = Db::open(temp.path().join("usage.db")).unwrap();
        db.manual_pricing()
            .save_price(&db, price("manual-target"))
            .unwrap();
        db.manual_pricing()
            .save_alias(
                &db,
            ...
            .unwrap();
        let result = db.manual_pricing().delete_price(
            &db,
            "manual-target",
